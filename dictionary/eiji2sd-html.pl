...
sub append_part {
    ...
    if ( defined $data->{'part'} ) {
        append( '【', escape( $data->{'part'} ), '】' );
    }
    append( '<dfn>', escape($word), '</dfn></dt>' );
}

sub append_extra {
    my $data = shift;
    if ( 0 != @{$data->{'note'}} or 0 != @{$data->{'example'}} ) {
        append '<ul style="list-style-type: none; margin-top: 0; margin-bottom: 0;">',
        ( map { '<li>' . escape($_) . '</li>' } @{$data->{'note'}}, @{$data->{'example'}} ),
        '</ul>';
    }
}
